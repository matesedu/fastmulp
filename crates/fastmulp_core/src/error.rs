  ...
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match self {
      ...
      Self::InvalidBoundaryByte { offset, byte } => {
        write!(f, "multipart boundary contains an invalid byte at {offset}: 0x{byte:02x}")
      }
      Self::InvalidStartingBoundary => f.write_str("multipart body does not start with the expected boundary"),
      Self::InvalidBoundaryTerminator { offset } => {
        write!(f, "multipart boundary terminator is invalid at byte offset {offset}")
      }
      Self::UnexpectedEnd { offset } => write!(f, "multipart body ended unexpectedly at byte offset {offset}"),
      Self::InvalidHeaderLineEnding { offset } => {
        write!(f, "multipart header line must end with CRLF at byte offset {offset}")
      }
      Self::InvalidHeaderContinuation { offset } => {
        write!(f, "multipart header continuation is not supported at byte offset {offset}")
      }
      Self::MissingHeaderSeparator { offset } => {
        write!(f, "multipart header is missing ':' at byte offset {offset}")
      }
      Self::InvalidHeaderName { offset } => write!(f, "multipart header name is invalid at byte offset {offset}"),
      Self::MissingContentDisposition { offset } => {
        write!(f, "multipart part is missing Content-Disposition at byte offset {offset}")
      }
      Self::MissingPartName { offset } => {
        write!(f, "multipart part is missing the Content-Disposition name parameter at byte offset {offset}")
      }
      ...
      Self::MissingClosingBoundary { offset } => {
        write!(f, "multipart closing boundary was not found after byte offset {offset}")
      }
      Self::TrailingData { offset } => write!(f, "multipart body has trailing data after byte offset {offset}"),
    }
  }
}
